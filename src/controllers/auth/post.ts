...
import { ErrorHandler, InternalServerError, sendOtp, verifyOtp, sendToken } from '@/utils';
...
export const Verify = catchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (reset !== undefined && reset !== null) {
      const isValid: boolean = typeof reset === 'boolean';
      if (!isValid) {
...
export const Login = catchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (email !== undefined && email !== null && email !== '') {
      ...
      if (user === null) {
        next(
          new ErrorHandler(StatusCode.BAD_REQUEST, 'Request Error', "phone number doesn't exist")
        );
        return;
      }
    } else {
