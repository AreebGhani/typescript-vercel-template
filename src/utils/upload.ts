...
const pdfCreateAsync = promisify(
  (html: string, options: pdf.CreateOptions, callback: (err: Error, buffer: Buffer) => void) => {
...
const getNewFileName = (filename: string, newExtension?: string): string => {
  ...
  const finalExtension =
    newExtension && newExtension.trim() !== '' ? newExtension : currentExtension;
  ...
  try {
    ...
    const files = (
      response.Contents?.map(file => file.Key).filter(
        (key): key is string => key !== undefined && !key.endsWith('/')
      ) || []
    ).map(file => {
...
const convertDocxToPdf = async (buffer: Buffer): Promise<Buffer> => {
  // Convert the DOCX buffer to HTML content
  const { value: html } = await mammoth.convertToHtml({ buffer });
  // Convert the HTML content to PDF and return the PDF as a buffer
  const pdfBuffer: Buffer = await pdfCreateAsync(html, { format: 'Letter' });
  return pdfBuffer;
};
...
const uploadMedia = async ({
  ...
  try {
    ...
    if (env.NODE_ENV !== 'production') {
      // eslint-disable-next-line no-console
      console.log(
        `\x1B[0;95m[StorageBucket]\x1B[0m [${new Date().toISOString()}] ${JSON.stringify(
          {
            request: {
    ...
    try {
      ...
      if (env.NODE_ENV !== 'production') {
        // eslint-disable-next-line no-console
        console.log(
          `\x1B[0;95m[StorageBucket]\x1B[0m [${new Date().toISOString()}] ${JSON.stringify({ response }, null, 2)}\n`
        );
      }
    } catch (err) {
