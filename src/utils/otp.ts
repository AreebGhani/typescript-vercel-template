...
import { isPhoneNumberVerified } from '@/firebase';
 ...
 */
export const OTP_RESEND_INTERVAL: number = 60 * 1000; // 1 Min
/**
 ** The duration (in milliseconds) for which a generated OTP remains valid.
 * Currently set to 1 minute (60,000 ms).
 */
export const OTP_EXPIRY_DURATION: number = 60 * 1000; // 1 Min
        ...
        if (timeElapsed < OTP_RESEND_INTERVAL) {
          next(
            new ErrorHandler(
              ...
              'Request Error',
              `please wait ${timeLeft} seconds before resending otp`
            )
          );
          return null;
        }
