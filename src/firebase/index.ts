 ...
 */
const firebase = (): admin.app.App => {
  try {
    return admin.apps.length && admin.app() !== undefined
      ? admin.app()
      : admin.initializeApp({
          credential: admin.credential.cert(
            path.join(path.resolve(), '/src/firebase/credential.json')
          ),
        });
  } catch (err) {
 ...
 */
const getUsers = async (): Promise<admin.auth.UserRecord[]> => {
 ...
 */
const isPhoneNumberVerified = async (phoneNumber: string): Promise<boolean> => {
  try {
