...
const setupSocketIO = (server) => {
     ...
     * };
     */
    const catchAsyncErrors = (theFunc) => (socket, next) => {
     ...
     */
    const logError = (socket, status, message) => {
    ...
    io.use(catchAsyncErrors(async (socket, next) => {
        try {
            const req = socket.request;
        ...
        try {
            ...
            if (userExist.isDeleted) {
                io.to(socket.id).emit('eventError', {
                    ...
                    message: 'your account has been permanently deleted. all associated data will be removed from our servers within 30 working days',
                });
                logError(socket, StatusCode.FORBIDDEN, 'your account has been permanently deleted. all associated data will be removed from our servers within 30 working days');
                return;
            }
            if (userExist.status !== 'active') {
                ...
                logError(socket, StatusCode.FORBIDDEN, `${userExist.role} account is currently inactive. please contact support for assistance`);
                return;
            }
