...
import { seeder, getUsedFiles } from './setup.js';
    ...
    mongoose.set('debug', (collectionName, methodName, ...methodArgs) => {
        if (env.NODE_ENV !== 'production') {
            if (collectionName.toLowerCase() !== 'transactions') {
                ...
                console.log(`\x1B[0;36m[Mongoose]\x1B[0m [${timestamp}] ${collectionName}.${methodName}(${formatArgs(methodArgs)})\n`);
            }
        }
    });
    // connect to MongoDB
...
const disconnect = () => {
    mongoose.connection
        ...
        .then(() => {
        // eslint-disable-next-line no-console
        console.log(`\x1B[0;36m[Mongoose]\x1B[0m [${new Date().toISOString()}] \x1b[31mDisconnected from MongoDB\x1B[0m\n`);
    })
        .catch(err => {
        // eslint-disable-next-line no-console
        console.log(`\n\x1B[31m[Error]\x1B[0m [${new Date().toISOString()}] Mongoose(${JSON.stringify({
